...
export default function VerifiedPage() {
  ...
  const error = (q.get('error') ?? '') as string;
  const errorCode = (q.get('error_code') ?? '') as string;
  const st = q.get('st') ?? '';            // present for student flow only
  const next = q.get('next') || '/signup/avatar'; // where to continue after finalize
    ...
    (async () => {
      try {
        ...
        if (!cancelled) {
          ...
          // gentle redirect so users see the success state
  ...
  // Error cases from Supabase link
  if (!ok) {
    return (
  ...
  // Student flow (token present): show finalize progress + auto-redirect to avatar
  if (st) {
    return (
      <div className="mx-auto max-w-lg p-6">
        <div className="rounded-xl border bg-white p-6 shadow-sm text-center">
          ...
          {finalizing && (
            <p className="text-gray-600 mb-6">{finalizeMsg}</p>
          )}
  ...
  // Normal flow (no token): keep your original UX to go to Login
  return (
    <div className="mx-auto max-w-lg p-6">
      <div className="rounded-xl border bg-white p-6 shadow-sm">
        <h1 className="text-2xl font-bold mb-2">Email verified 🎉</h1>
